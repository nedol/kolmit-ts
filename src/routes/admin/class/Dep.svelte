<script lang="ts">
  ...
  let OnClickUser = getContext('OnClickUser');
  ...
  interface Department {
    staff: Staff[]; // предположим, что Staff - это интерфейс для объектов с полем name
    // Другие свойства, если есть
  }

  export let dep: Department = {
    staff: [], // инициализация пустым массивом, если нужно
    // Остальные свойства объекта
  };

  function sortStaffAlphabetically(staff: Staff[]) {
    if (staff && staff.length > 0) {
      return staff.sort((a: Staff, b: Staff) => a.name.localeCompare(b.name));
    } else {
      return [];
    }
  }
	...
	<button class="collapsible" {owner} bind:this={button} on:click={OnCollClick}>
		...
		{#if edited_display}
			{#if operator.email === operator.abonent}
				<svg height="30" width="30" style="position: relative;float:right">
...
<div class="content" bind:this={content}>
  ...
  {#if dep.staff}
    ...
    <div class="image-row">
      {#each dep.staff as user, u}
        ...
        <div class="image-wrapper" on:click={OnClickUser}>
          <Card style="height: 50px;width:50px">
            <Image
              name={user.name}
              email={user.email}
              style="
				...
				height: 40px;
			"
              src={user.picture ? user.picture : operator_svg}
              alt="Image {u + 1}"
            />
          ...
          <span style="font-size: smaller; text-align:center">{user.name}</span> -->
        </div>
      {/each}
    </div>
  {/if}
  {#if edited_display}
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div class="add_oper" style="display:{isAddOper}">
