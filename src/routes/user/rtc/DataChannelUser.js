...
import { dc_user, dc_state, msg, posterst } from '$lib/js/stores.js';
let poster;
...
export class DataChannelUser {
  ...
  SendFile(data, name, resolve) {
    ...
    try {
      if (this.dc.readyState === 'open') {
        ...
        this.dc.send(
          JSON.stringify({ file: name, length: data.byteLength, from: 'user' }),
          function (data) {
            console.log(data);
          }
        );
  ...
  SendData(data, cb) {
    ...
    try {
      if (this.dc.readyState === 'open') {
        ...
        for (let i = 0, o = 0; i < numChunks; ++i, o += size) {
          this.dc.send(
            JSON.stringify({ slice: data.substr(o, size), from: 'user' })
          );
        }
  ...
  SendDCCall() {
    ...
    par.profile = {
      email: that.rtc.oper.operator,
      name: that.rtc.oper.name,
      img: that.rtc.oper.picture,
    };
