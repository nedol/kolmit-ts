<script lang="ts">
  ...
  import { llang, langs, dicts, lesson, dc } from "$lib/stores.ts";
  ...
  onMount(async () => {
    try {
      ...
      words = result?.data.data.map((item) => ({
        example: { nl: item.example.nl },
      }));
      currentWordIndex = 0;
  ...
  function nextWord() {
    if (currentWordIndex < words.length - 1) currentWordIndex++;
...
{#if words}
  ...
  <div class="container">
    <span
      ...
      font-style: italic;
      font-size: smaller;
      font-family: serif;">{data.name}</span
    >
