<script lang="ts">
  ...
  const handleClick = (word: Word) => {
    ...
    if (cor_word === word.value.toLowerCase().replace(/<[^>]*>/g, "")) {
      ...
      requestAnimationFrame(() => {
        const nextElement = document.querySelectorAll(".formatted-list span")[
          focusedIndex
        ];
        if (nextElement) {
          nextElement.focus();
  ...
  function getCorrectSpanString(isCorrect) {
    const elements = document.querySelectorAll(
      isCorrect
        ? ".formatted-list > .correct"
        : ".formatted-list > span:not(.correct)"
    );
  ...
  const SpeakText = async (text = "") => {
    ...
    const textToSpeak = getCorrectSpanString(
      isSTT || formattedSentence.every((item) => item.class === "correct")
    );
  ...
  function SttResult(text) {
    ...
    if (numbers)
      sent_compare = sent_compare.replace(
        /\d+/g,
        numberToDutchString(numbers[0])
      );
  ...
  function PlayAutoContent() {
    ...
    async function endSpeak() {
      ...
      tts.Speak_server(
        $langs,
        bricks_data.translate[curSentence],
        data.name,
        endLangSpeak
      );
    }
...
{:else}
      ...
      <Row>
        ...
        <Section align="start">
          ...
          {:else}
            ...
            {#if isSynt === false}
              <div
                style="position:absolute; display: flex; justify-content: center;pointer-events: none;"
              >
                <CircularProgress
  ...
  <Container>
    ...
    <div class="rate">
      <p>
        <span class="mdc-typography--overline">{rate.cnt.toFixed(0)} </span>
      </p>
    </div>
    ...
    <div class="formatted-list">
      {#each formattedSentence as item, index}
        <span
          class={`${item.class} ${item.gr}`}
          tabindex="0"
          value={item.value.replace(/<[^>]*>/g, "")}
          on:click={() => {
            item.word = "";
            focusedIndex = index;
            handleFormatted(item, index);
          }}
    ...
    <div>
      ...
      <div class="word-list">
        ...
        {#each words as word, index}
          <!-- {@debug word} -->
          <span class={word.gr} on:click={() => handleClick(words[index])}
            >{@html word.value}</span
          >
        {/each}
      </div>
    </div>
...
<style>
  ...
  .word-list .verb,
  .formatted-list .verb:not(.invisible) {
  ...
  .word-list .subj,
  .formatted-list .subj:not(.invisible) {
    color: rgb(49, 49, 169);
  }
  ...
  .word-list .tijd,
  .formatted-list .tijd:not(.invisible) {
  ...
  .word-list .plaats,
  .formatted-list .plaats:not(.invisible) {
    color: darkmagenta;
  }
  ...
  .word-list .adv,
  .formatted-list .adv:not(.invisible) {
    color: darkmagenta;
  }
  ...
  .word-list .obj,
  .formatted-list .obj:not(.invisible) {
  ...
  .word-list .ptcl,
  .formatted-list .ptcl:not(.invisible) {
  ...
  .word-list .conj,
  .formatted-list .conj :not(.invisible) {
    color: rgb(184, 162, 40);
  }
